...
class PaiementController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            return redirect()->route('admin.paieme_index')->with('success', 'Paiement ajouté avec succès!');
        } catch (\Exception $e) {
...
//         'id_beneficiaire' => 'required|exists:beneficiaires,id',
//         'status' => 'required|string',
//         'motif_de_la_depence' => 'required|string',
//         'impulsion' => 'required|string',
//     ]);
...
public function update(Request $request, $id)
{
        ...
        return redirect()->route('admin.paieme_index')->with('success', 'Paiement mis à jour avec succès.');
    ...
    public function destroy($id)
    {
        try {
            ...
            return redirect()->route('admin.paieme_index')->with('success', 'Paiement supprimé avec succès.');
        } catch (\Exception $e) {
            return redirect()->route('admin.paieme_index')->with('error', 'Une erreur s\'est produite lors de la suppression du paiement.');
        }
    }
    ...
    public function import(Request $request)
    {
        try {
            ...
            return back()->with('error', 'Une erreur est survenue lors de l\'importation des paiements : ' . $e->getMessage());
        }
    }
...
function afficherAnneeActuelle() {
    ...
    $section->addText("Mode de paiement : " . " " . $paiement->mode_paiement, null, ['bold' => true]);    $section->addText("Montant brut : " . $this->convertirMontantEnLettres($paiement->montant) . " MRU", ['bold' => true], ['align' => Jc::LEFT]);
 ...
 $section->addText("Motive de la dépense : "." " .$paiement->motif_de_la_depence, ['bold' => true], ['align' => Jc::LEFT]);
  ...
  $section->addText("Motive de la dépense : "." " .$paiement->motif_de_la_depence, ['bold' => true], ['align' => Jc::LEFT]);
    ...
    $section->addText("Montant (en chiffres) : " . number_format($paiement->montant, 0, ',', ' ') . " MRU", ['bold' => true], ['align' => Jc::LEFT]);
    $section->addText("Montant (en lettres) : " . $this->convertirMontantEnLettres($paiement->montant) . " MRU", ['bold' => true], ['align' => Jc::LEFT]);
    $section->addText("Mode de paiement : " . " " . $paiement->mode_paiement, null, ['bold' => true]);
